...
public class LoopEx {
	public static void whileEx() {
		// whle 연습
		//100이하의 정수를 합산하여 결과 출력
		int num = 1; // 반복조건을 확인하기위한 제어변수
		int total = 0; //합산변수
	...
	public static void continueEx() {
		//continue 반복문에서 남아있는 코드를 더이상 실행 하지 않고
		//다음 루프를 진행
	...
	public static void breakEx() {
		//break문은 반복문을 종료하고 반복문 다음 문장으로 이도
		//1과 차례로 반복하면서 
		//6과 14로 동시에 나누어 떨어지는 수를 출력
		//루프를 언제까지 돌려야 할지 알수 없으므로 while문
		int num = 1;
		while(true) { //무한루프
			if(num%6==0 && num%14==0) {
				//찾았다!
